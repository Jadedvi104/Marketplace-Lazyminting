...
contract BWNFTMarket is ReentrancyGuard, ERC721Holder, Pausable, AccessControl {
    ...
    function compareTimeEnd(uint256 tokenId)
        public
        view
        returns (Offer[] memory)
    {
        ...
        for (uint32 i = 0; i < offers[tokenId].length; i++) {
            if (
                block.timestamp <= offers[tokenId][i].timeOfferEnd ||
                offers[tokenId][i].active == true
            ) {
                length++;
            }
        }
        ...
        for (uint32 j = 0; j < offers[tokenId].length; j++) {
            if (
                block.timestamp <= offers[tokenId][j].timeOfferEnd ||
                offers[tokenId][j].active == true
            ) {
                availableOffers[x] = offers[tokenId][j];
                x++;
            }
        }
